...
#ifdef CONFIG_OPLUS_ION_BOOSTPOOL
	struct ion_boost_pool* boost_pool = cached ? heap->cached_boost_pool : NULL;
				...
				unsigned long align, unsigned long flags)
{
	...
	*sys_heap = container_of(heap,
	...
	struct ion_boost_pool *boost_pool =
		cached ? sys_heap->cached_boost_pool : NULL;
...
struct ion_heap *ion_mm_heap_create(struct ion_platform_heap *unused)
{
	...
	if (unused->id == ION_HEAP_TYPE_MULTIMEDIA_FOR_CAMERA &&
		...
		unsigned long cam_sz = SZ_32M >> PAGE_SHIFT;

		if (!kcrit_scene_init())
			pr_err("%s: init kcrit scene failed!\n",
			       __func__);

		/* for ram > 4G set 128M for camera */
		if (totalram_pages > (SZ_1G >> PAGE_SHIFT) * 4)
			cam_sz = SZ_128M >> PAGE_SHIFT;
