...
#define EEPROM_SENSORID_S5KJN1SP   0x06
#define S5KJN1SP_XTALK_DATA_FLAG   0x24D4

#define S5KJN1SP_XTALK_START_ADDR1 0x1AA0
#define S5KJN1SP_XTALK_DATA_SIZE1   2612
#define S5KJN1SP_XTALK_START_ADDR2 0x2500
#define S5KJN1SP_XTALK_DATA_SIZE2   768
#define S5KJN1SP_XTALK_START_ADDR3 0x2810
#define S5KJN1SP_XTALK_DATA_SIZE3   4000
#define S5KJN1SP_XTALK_START_ADDR4 0x3860
#define S5KJN1SP_XTALK_DATA_SIZE4   626

#define S5KJN1SP_XTALK_DATA_SIZEALL   8006

//static char S5KJN1sp_data_xtalk1[2612];
//static char S5KJN1sp_data_xtalk2[768];
//static char S5KJN1sp_data_xtalk3[4000];
//static char S5KJN1sp_data_xtalk4[626];
static char S5KJN1sp_data_xtalkall[8008];

static unsigned int xtalk_readed = 0;
static void read_4cell_from_eeprom_s5kjn1(void);
#endif
...
{
	int i =0;
	kal_uint16 xtalk_flag = 0;
	kal_uint16 Sensor_ID = 0;
	if (!xtalk_readed) {
		Sensor_ID = read_cmos_eeprom_8(EEPROM_CHECK_ID_ADDR);
		printk("%s Sensor_ID = %d\n",__FUNCTION__,Sensor_ID);
		if (Sensor_ID == EEPROM_SENSORID_S5KJN1SP) {
			xtalk_flag = read_cmos_eeprom_8(S5KJN1SP_XTALK_DATA_FLAG);
			printk("%s xtalk_flag = %d\n",__FUNCTION__,xtalk_flag);
			if (xtalk_flag == 1) {

				for (i = 0; i < S5KJN1SP_XTALK_DATA_SIZE1; i++) {
					S5KJN1sp_data_xtalkall[2 + i] = read_cmos_eeprom_8(S5KJN1SP_XTALK_START_ADDR1 + i);
				}
				for (i = 0; i < S5KJN1SP_XTALK_DATA_SIZE2; i++) {
					S5KJN1sp_data_xtalkall[2614 + i] = read_cmos_eeprom_8(S5KJN1SP_XTALK_START_ADDR2 + i);
				}
				for (i = 0; i < S5KJN1SP_XTALK_DATA_SIZE3; i++) {
					S5KJN1sp_data_xtalkall[3382 + i] = read_cmos_eeprom_8(S5KJN1SP_XTALK_START_ADDR3 + i);
				}
				for (i = 0; i < S5KJN1SP_XTALK_DATA_SIZE4; i++) {
					S5KJN1sp_data_xtalkall[7382 + i] = read_cmos_eeprom_8(S5KJN1SP_XTALK_START_ADDR4 + i);
				}

				S5KJN1sp_data_xtalkall[0] = 0;
				S5KJN1sp_data_xtalkall[1] = 8;

				/*for (i = 0; i < S5KJN1SP_XTALK_DATA_SIZEALL + 2; i++) {
					printk("read eeprom %d,0x%x\n",i,S5KJN1sp_data_xtalkall[i]);
				}*/

				xtalk_readed = 1;
			} else {
				printk("Warning, S5KJN1sp'otp not have cross-talk calibration data !\n");
			}
		}
...
unsigned int read_4cell_data_from_eeprom(char *data)
{
	int flag_xtc = 0;
	if (data != NULL) {
		memcpy((void*)(data), (void*)S5KJN1sp_data_xtalkall, S5KJN1SP_XTALK_DATA_SIZEALL + 2);
	}
	flag_xtc = read_cmos_eeprom_8(0x2800);
	printk("[read_4cell_data_from_eeprom]flag_xtc = %d\n",flag_xtc);
	return 0;
}
#endif
...
UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/*get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_ALL) {
			...
			read_4cell_data_from_eeprom((char *)(*(feature_data+1)));
