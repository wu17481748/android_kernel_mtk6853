...
static int tri_key_m1120_i2c_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	TRI_KEY_LOG("allocation memory for p_m1120_data up %s  up\n", __func__);
	...
	TRI_KEY_LOG("init mutex variable up\n");
	/*(3) config i2c client*/
	...
	if (gpio_is_valid(p_data->irq_gpio)) {
		...
		if (err) {
			TRI_KEY_LOG("unable to request gpio [%d]  up", p_data->irq_gpio);
		} else {
			err = gpio_direction_input(p_data->irq_gpio);
			msleep(50);
			p_data->irq = gpio_to_irq(p_data->irq_gpio);
			TRI_KEY_LOG("======> irq : %d  up", p_data->irq);
		}
	}
	...
	TRI_KEY_LOG("%s was found  up", id->name);
	...
	p_m1120_data->source = wakeup_source_register(NULL, "m1120 wakeup source hall up");
	if (p_m1120_data->source == NULL) {
		TRI_KEY_ERR("m1120 wakeup source hall up register fail \n");
	}
