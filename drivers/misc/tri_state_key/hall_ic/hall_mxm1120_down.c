...
static int tri_key_m1120_i2c_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	TRI_KEY_LOG("allocation memory for p_m1120_data down %s  down\n", __func__);
	...
	TRI_KEY_LOG("init mutex variable down\n");
	/*(3) config i2c client*/
	...
	if (client->dev.of_node) {
		TRI_KEY_LOG("Use client->dev.of_node  down\n");
	...
	if (gpio_is_valid(p_data->irq_gpio)) {
		...
		if (err) {
			...
			TRI_KEY_LOG("======> irq : %d  down", p_data->irq);
		}
	}
	...
	TRI_KEY_LOG("%s was found  down", id->name);
	...
	TRI_KEY_LOG("%s was initialized  down", M1120_DRIVER_NAME_DOWN);
	...
	TRI_KEY_LOG("%s : %s was probed. down\n", __func__, M1120_DRIVER_NAME_DOWN);
